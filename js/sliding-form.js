(function($){
    $.fn.sidePanelForm = function (options) {
        var settings = $.extend({
            title   : 'Get Your Free<br>20 Min. Therapy Planning Call',
            color   : '#0059af'
        }, options);

        if(settings.form_id){
            if($(settings.form_id).length){
                $('body').append('<a href="'+settings.form_id+'" class="request_consultation">Get Your Free Therapy Planning Call</a>\
        ...
        function createForm(formId){
            ...
            $("<link/>", {
                rel: "stylesheet",
                type: "text/css",
                href: "http://lp.resultsmattercorp.com/nsw-carpet-cleaning-group/css/sliding-form.css"
             }).appendTo("head");
